    ...
    if tag in content:
        ...
        if len(parts) > 1:
            ...
            if len(sub_parts) > 1:
                list_html = ""
                if not news_items:
                    list_html = "<li class='news-item' style='color:#7A7571;'>近 3 日內無相關新聞。</li>"
...
def update_html_price_row(content, row_id, change_val, is_us=True):
    """
    根據台美股不同慣例，動態改寫前端 漲跌幅 欄位的顏色
    美股: 正=綠(#2E7D32), 負=紅(#C62828)
    台股: 正=紅(#C62828), 負=綠(#2E7D32)
    """
    tag = f'id="{row_id}">'
    if tag in content:
        parts = content.split(tag)
...
def main():
    ...
    # 雙源依個股精密新聞歸檔 (00981A 使用全名關鍵字精確掃描)
    ...
    log_msg("✨ 網頁色彩優化與高階 FIRE 計算機整合大成功！")
